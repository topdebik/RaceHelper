...
class mainWindowApp(QtWidgets.QMainWindow, mainWindow.Ui_MainWindow):
    def __init__(self):
        try:
            ...
            self.btnChooseCurrentPath.clicked.connect(
                self.browseFolderAndShowParticipants
            )
    ...
    def browseFolderAndShowParticipants(self):
        try:
            ...
            directory = QtWidgets.QFileDialog.getOpenFileName(
                self, "Выберите файл", "", "Excel Files (*.xlsx);;All Files (*)"
            )
            if directory[0] != "":
    ...
    def showCompletedAlert(self):
        try:
            ...
            if "directory" not in globals() or directory[0] == "":
                ...
                if (
                    "Results" in str(self.window)
                    and "resultsDirectory" not in globals()
                ):
                    self.message.about(
                        self, "Генератор", "Файл с конфигурацией очков не выбран"
                    )
                else:
                    if "resultsNotFound" in globals():
                        ...
                        del resultsNotFound
                        self.message.about(
                            self, "Генератор", "Результаты по заездам не были найдены"
                        )
                    else:
    ...
    def browseFolderAndSelectResultsConfig(self):
        try:
            ...
            resultsDirectory = QtWidgets.QFileDialog.getOpenFileName(
                self.window, "Выберите файл", "", "Text Files (*.txt);;All Files (*)"
            )
    ...
    def showToolGeneratePlacesWindow(self):
        ...
        try:
            ...
            self.window.boxRunsPerParticipant.valueChanged.connect(
                self.valueChangePlaces
            )
        except Exception as e:
            showErrorMessage(e)
            ...
            try:
                if "directory" in globals() and directory[0] != "":
                    ...
                    if generateMethod == "До максимального веса":
                        weights_func.weightsCalcMaxPerson(
                            str(directory[0]), weightTick, maxKartWeight
                        )
                    elif generateMethod == "До указанного веса":
                        weights_func.weightsCalcFixed(
                            str(directory[0]), weightTick, maxKartWeight, weightGoal
                        )
            except Exception as e:
                showErrorMessage(e)
    ...
    def showToolGenerateResultsWindow(self):
        def results_func_activate():
            try:
                if (
                    "directory" in globals()
                    and directory[0] != ""
                    and "resultsDirectory" in globals()
                ):
                    results_func.resultsCalcCustom(
                        str(directory[0]), str(resultsDirectory[0])
                    )
            except Exception as e:
        ...
        try:
            ...
            self.window.show()
            self.window.btnChooseCurrentPath.clicked.connect(
                self.browseFolderAndSelectResultsConfig
            )
...
class toolGeneratePlacesWindowApp(
    QtWidgets.QMainWindow, toolGeneratePlacesWindow.Ui_toolGeneratePlacesWindow
):
    def __init__(self):
        try:
            super().__init__()
...
class toolGenerateWeightsWindowApp(
    QtWidgets.QMainWindow, toolGenerateWeightsWindow.Ui_toolGenerateWeightsWindow
):
    def __init__(self):
        try:
            super().__init__()
...
class toolGenerateResultsWindowApp(
    QtWidgets.QMainWindow, toolGenerateResultsWindow.Ui_toolGenerateResultsWindow
):
    def __init__(self):
        try:
            super().__init__()
...
def showErrorMessage(e):
    ...
    errDialog.setInformativeText(
        "Error occured: \042%s\042. Application will now close." % str(e)
    )
...
def main():
    localWeb = Process(target=web.startLocalWeb)
    publicWeb = Process(target=web.startPublicWeb)
    localWeb.start()
    publicWeb.start()
